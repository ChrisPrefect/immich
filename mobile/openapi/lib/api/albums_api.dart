...
class AlbumsApi {
  ...
  ///
  /// * [String] id (required):
  Future<SharingPermissionsResponseDto?> getOwnAlbumUser(String id,) async {
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'SharingPermissionsResponseDto',) as SharingPermissionsResponseDto;
  ...
  /// * [String] id (required):
  ///
  /// * [UpdateSharingPermissions] updateSharingPermissions (required):
  Future<Response> updateOwnAlbumUserWithHttpInfo(String id, UpdateSharingPermissions updateSharingPermissions,) async {
    // ignore: prefer_const_declarations
    final apiPath = r'/albums/{id}/user/self'
      .replaceAll('{id}', id);

    // ignore: prefer_final_locals
    Object? postBody = updateSharingPermissions;
  ...
  ///
  /// * [UpdateSharingPermissions] updateSharingPermissions (required):
  Future<void> updateOwnAlbumUser(String id, UpdateSharingPermissions updateSharingPermissions,) async {
    final response = await updateOwnAlbumUserWithHttpInfo(id, updateSharingPermissions,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
  }
